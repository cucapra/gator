...
and eval_comm (c : comm) (fns : fn list) (s : sigma) : sigma =
  ...
  | If ((b, c1), el, c2) -> (
      let check_if b s =
        ...
        | _ ->
            failwith
              "Typechecker failure: expected a boolean in 'if' expression"
      in
