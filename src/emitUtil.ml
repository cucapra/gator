...
let replace_type_in_constrain (c : constrain) (t : etyp) (r : etyp) : constrain =
    match c with
    | ETypConstraint et -> ETypConstraint (replace_type et t r)
    | _ -> c
...
let rec replace_type_in_parameterization (pm : parameterization) (t : etyp) (r : etyp) : parameterization =
    ...
    Assoc.gen_context (List.map (fun (n, c) -> match t with AbsTyp(x, _) -> if x = n then (x, ETypConstraint r) else (n, replace_type_in_constrain c t r) | _ -> (n, replace_type_in_constrain c t r)) pm_list)
...
let rec list_of_constraint (c : constrain) (int_and_float : bool) : etyp list =
    match c with
...
let generate_generics_for_fn ((s, (p, rt, pm)), cl : fn) (int_and_float : bool) : fn list =
    ...
    let rec replace_generic (orig: fn list) (pml : (string * constrain) list) : fn list =
        match pml with
        | [] -> orig
        | (s,c)::tl -> 
            let lst = list_of_constraint c int_and_float in
            let result = List.fold_left (fun acc t -> debug_print ((string_of_typ t)); replace_type_in_fn_list orig (AbsTyp(s,c)) t @ acc) [] lst in
